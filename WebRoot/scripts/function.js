...
$(function(){
    ...
    $(".number").find("span").click(function(){
        ...
        window.location='Cart?action=mod&entityId='+data[0]+'&quantity='+number+'&index='+data[1];
    });
    //计算总价
    function totalPrice(){
        var totalPrice=0;
    ...
    $("#product .thumb img").mouseover(function(){
        ...
        $("#product").append('<img id="bigImg" src="files/'+name+'_big.jpg"/>');
    ...
    $("#shoppingBag").mouseover(function(){
        ...
        $("#header").append($table);
        var html=["<tr><th>图标</th><th>名称</th><th>价格</th></tr>"];        
        var data=eval(cart);
        if(data.length<=0){
            html=[];
        }
        for(var i=0;i<data.length;i++){
        	var d = data[i];
        	html.push("<tr><td><img width='25' height='25' src='"+d[0]+"'/></td><td>"+d[1]+"</td><td>"+d[2]+"</td></tr>");
        }
